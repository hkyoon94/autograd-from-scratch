...
def generate_toy_data(
    ...
    axes: list[plt.Axes]
    axes[0].scatter(x, y1, s=2)
    axes[1].scatter(x, y2, s=2)
    axes[0].set_title("$f_1(\mathbf{x}_i)$", fontsize=12)
    axes[1].set_title("$f_2(\mathbf{x}_i)$", fontsize=12)
...
class DataLoader:
    ...
    def __init__(self, data: MyDataset, bsz: int):
        self.data = data
        self.bsz = bsz
